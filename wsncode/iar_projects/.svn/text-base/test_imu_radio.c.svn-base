/*
 * This is a standalone program which retrieves all the IMU sensors values on the
 * GINA2.2b/c board: gyro, large range accel, magnetometer, sensitive accel,
...
void task_application_imu_radio(uint16_t input) {
   ...
   */
   ...
   P1OUT |= 0x04;P1OUT &= ~0x04;                 // pulse P1.2 for debug
   //packet number
   packetfunctions_reserveHeaderSize(testImuRadiopacketToSend,2);
   packetfunctions_htons(counter, &(testImuRadiopacketToSend->payload[0]));
...
void radio_sendDone(OpenQueueEntry_t* pkt, error_t error) {
