...
void cmd_loop (void) {
  ...
  if (cmd_mode != CMD_MODE_WAIT && cmd_mode != CMD_MODE_SPIN && cmd_mode != CMD_MODE_ERROR) {
  ...
  if (cmd_mode == CMD_MODE_SPIN) spinonce(2000,50,&at_state, AT_STATE_RX_WAITING);
  if (cmd_mode == CMD_MODE_ERROR) erroronce(2000,150,&at_state, AT_STATE_RX_WAITING);
